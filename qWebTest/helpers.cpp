...
{
    bool bOneLiner = ((iLevel == LOG_INF) && (s.length() == 1));
    QString sLevel("ERR");
    {
    ...
    if(bDebug)
    {
        ...
        FILE *fp = fopen(sLogFN.toLatin1(), "a+"/*append*/);
        if(!fp)
        {
            sLogFN = QDir::tempPath()+"/.log";
            fp = fopen(sLogFN.toLatin1(), "a+"/*append*/);
            if(!fp)
                return;
        }
        //fputs(QString(sDate+(bWithDate ? " " : "")+sLevel+" "+s+"\n").toAscii(),fp);
        static bool bLastPutWasOneLiner = false;
        if(bOneLiner)
        {
            fputs(s.toLatin1(),fp);
            bLastPutWasOneLiner = true;
        }
        else
        {
            fputs(QString((bLastPutWasOneLiner ? "\n":"")+sDate+" "+sLevel+" "+s+"\n").toLatin1(),fp);
            bLastPutWasOneLiner = false;
        }
        fclose(fp);
    }
}
...
int helpers::copydirifdifferent(QString sFr, QString sTo, QApplication* pApp, QObject* pLogTarget)
{
    int iRe = 0;
    QDir src(sFr);
    QDir dst(sTo);
    ...
    foreach (QFileInfo fileInfo, contents)
    {
        ...
        if (fileInfo.isDir())
        {
            ...
            iRe += copydirifdifferent(srcFilePath, dstFilePath, pApp, pLogTarget);
        }
        else if (fileInfo.isFile())
        {
...
QString helpers::getarg(QString sEntryName, QApplication* pApp, QObject* pLogTarget)
{
    ...
    for(int i=0; i < pApp->arguments().size(); i++)
    {
        QString sarg(pApp->arguments().at(i));
        sarg = sarg.trimmed();
        int iE = sarg.indexOf('=');
        if (iE > 0)
        {
...
int helpers::copydirifdifferent(QString sFrom,QString sTo,QString sPattern, bool bWarnIfNotFound, QApplication* pApp, QObject* pLogTarget)
{
    int iRe = 0;
    QDir dfrom(sFrom);
    QDir dto(sTo);
    ...
    for(int i=0; i < fs.length(); i++)
    {
        ...
        if(f.isDir())
        {
            ...
            iRe += copydirifdifferent(sFNFrom, sFNTo, sPattern, bWarnIfNotFound, pApp, pLogTarget);
        }
        else
        {
...
bool helpers::str_isnum(QString s, bool bCanHaveDot, bool bCanBeNegative)
{
    if(helpers::str_isempty(s, true))return false;
    s = s.trimmed();

    bool ok;
    if(bCanHaveDot && (s.indexOf('.') > 0))
    {
        s.toDouble(&ok);
        if(!ok)return false;
    }
    ...
    static QRegExp re("\\d+");  // a digit (\d), 1 or more times (+)
    if(re.exactMatch(s))
        return true;
    return false;
}
